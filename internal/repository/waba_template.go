...
func (r *WABATemplateRepository) GetByID(ctx context.Context, id uuid.UUID) (*WABATemplate, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, workspace_id, meta_template_id, name, language, status, category, components, created_at, updated_at
		 FROM waba_templates WHERE id = $1`,
		id,
	).Scan(&tmpl.ID, &tmpl.WorkspaceID, &tmpl.MetaTemplateID, &tmpl.Name, &tmpl.Language, &tmpl.Status, &tmpl.Category, &tmpl.Components, &tmpl.CreatedAt, &tmpl.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &tmpl, nil
...
// GetByNameAndLanguage retrieves a template by workspace, name, and language.
func (r *WABATemplateRepository) GetByNameAndLanguage(ctx context.Context, workspaceID uuid.UUID, name, language string) (*WABATemplate, error) {
	var tmpl WABATemplate
	err := r.pool.QueryRow(ctx,
		`SELECT id, workspace_id, meta_template_id, name, language, status, category, components, created_at, updated_at
		 FROM waba_templates WHERE workspace_id = $1 AND name = $2 AND language = $3`,
...
func (r *WABATemplateRepository) ListByWorkspace(ctx context.Context, workspaceID uuid.UUID) ([]WABATemplate, error) {
	...
	for rows.Next() {
		var tmpl WABATemplate
		if err := rows.Scan(&tmpl.ID, &tmpl.WorkspaceID, &tmpl.MetaTemplateID, &tmpl.Name, &tmpl.Language, &tmpl.Status, &tmpl.Category, &tmpl.Components, &tmpl.CreatedAt, &tmpl.UpdatedAt); err != nil {
			return nil, err
		}
