...
| **pgx/v5** | **v5.10.0** (`github.com/jackc/pgx/v5`) | PostgreSQL driver (native path) | Binary protocol, prepared-statement cache, native `COPY`, `Batch`/`QueryExecMode` pipeline — the right tool for the audit batch writer. `CollectRows`/`ForEachRow` helpers replace an ORM. Use a single shared `*pgxpool.Pool` for OmniGo's own queries. |
