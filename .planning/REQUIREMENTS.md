...
- [ ] **QUEUE-01**: NATS JetStream work-queue stream for outbound message durability (at-least-once, single consumer per subject)
- [ ] **QUEUE-02**: Backpressure: 1,000-message per-session limit enforced before enqueue with HTTP 429/422 + `Retry-After` header
- [ ] **QUEUE-03**: Dedup mechanism: `Nats-Msg-Id = trace_id` for publish-side idempotency + `dispatched_messages` dedup set to prevent duplicate delivery on redelivery
