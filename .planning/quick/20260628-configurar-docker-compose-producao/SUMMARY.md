...
2. **Parameterized PostgreSQL Credentials**:
   - Replaced hardcoded PostgreSQL username, password, and database name with environment variables `${POSTGRES_USER}`, `${POSTGRES_PASSWORD}`, and `${POSTGRES_DB}` with default values fallback (`postgres`/`postgres`/`omnigo`).
   - Cleaned up healthcheck and `OMNIGO_DATABASE_URL` strings to dynamically use these variables.
3. **Verification**:
